...
adaptIntegrate(f1, lowerLimit = c(0,0), upperLimit = c(pi/4, pi/2))$integral
```
...
```{r linewidth=80}
samp = sample(1:100, 100000, replace = TRUE)
mat1 = matrix(samp, nrow = 1000, ncol = 100)
mat2 = vector('list', 1000)
for (n in 1:1000) {
  num = length(unique(mat1[n,]))
  mat2[n] = num
}
mat2 = unlist(mat2)
hist(mat2, main = 'Unique Integers From Sample Of 100 Integers')
mean(mat2)
sd(mat2)
```
...
a = c(0.4, -3, 4)
f2 = function(x, y, z) {
  sum((z - (x[1]*y^2 + x[2]*y + x[3]))^2)
}
gradient = function(x, y, z) {
  n = x[1]*y^2 + x[2]*y + x[3] - z
  n1 = sum(2*n*(y^2))
  n2 = sum(2*n*y)
  n3 = sum(2*n)
  return(c(n1,n2,n3))
}
output = suppressWarnings(optim(a, f2, gr = gradient, x = x, y = y, method = 
                                  'BFGS'))
output$par
plot(x,y)
xval = seq(min(x), max(x), by = 0.01)
lines(x = xval, y = output$par[1]*xval^2 + output$par[2]*xval + output$par[3])
```
...
matchDict = function(x) {
  if(is.character(x) == FALSE) {
    ...
    let = unlist(strsplit(x, ''))
    perm = permn(let)
    for (i in 1:length(perm)) {
      w = paste(unlist(perm[i]), collapse = '')
      w = tolower(w)
      if (w %in% GradyAugmented) {
        wordL[i] = w
      }
    }
    wordL = wordL[!is.na(wordL)]
    wordL = unique(wordL)
    return(wordL)
  }
}
...
rand = vector('list', 10000)
for (i in 1:10000) {
  samp = as.vector(sample(x, length(x), replace = FALSE))
  rand[[i]] = samp
}
sdiff = vector()
for (i in 1:length(rand)) {
  sdiff[i] = diff(tapply(y, rand[[i]],mean))
}
sum(sdiff > observed.diff)/10000
```
```
The p-value of the mean difference of the 10,000 populations is less than 0.05.
Hence, we reject the null hypothesis that the real difference in population means is 0.
```
