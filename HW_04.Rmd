...
apply(mat, 1, function(x) {
  return(sqrt((sum((x - mean(x))**2, na.rm = TRUE)/length(x))))
})
```
...
avg.une.inf.sv = function(strikes.df) {
  list(unemployment = mean(strikes.df$unemployment),
       inflationRate = mean(strikes.df$inflation),
       strikeVolume = mean(strikes.df$strike.volume))
}
dim(strikes.df %>% split(., .$year) %>% sapply(., avg.une.inf.sv))
strikes.df %>% split(., .$year) %>% sapply(., avg.une.inf.sv) %>% .[,c('1962', '1972', '1982')]
```
...
```{r linewidth=80}
library(tidyverse)
strikes.df %>% group_by(.$country) %>% summarize(ave_unemployment = mean(unemployment)) %>% arrange(., desc(ave_unemployment)) %>% slice(1, n())
```
