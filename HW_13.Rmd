...
**output**
HW_13.Rmd        Previous        player-stats.sql
Homework12       README
...
```
**input**
select count(distinct college) from player_bios_ where college != 'None';
...
```
**input**
create type DivisionType as enum('Atlantic', 'Central', 'Southwest', 'Northwest', 'Southeast');
...
**input**
create table teams (id char(3), location text, name text, division DivisionType, conference ConferenceType);
...
```
**input**
create table new_table (player integer references more_player_stats(id), prl numeric, position text);
...
**input**
update new_table set position = 'PF'::text where prl >= 11.3;

**output**
UPDATE 110

**input**:
update new_table set position = 'PG'::text where prl >= 10.8 and prl < 11.3;

**output**:
UPDATE 92

**input**:
update new_table set position = 'C'::text where prl >= 10.6 and prl < 10.8;

**output**
UPDATE 80

**input**
update new_table set position = 'SG/SF'::text where prl >= 0 and prl < 10.6;
...
**input**
update player_biosset position = (select position from new_table where id = player);
...
```
**input**
alter table player_bios add column height_i numeric;

**ouput**
ALTER TABLE

**input**
update player_bios set height_i = 12*split_part(height,'-',1)::integer + split_part(height,'-',2)::integer;
...
**input**
alter table player_bios rename column height_i to height;
